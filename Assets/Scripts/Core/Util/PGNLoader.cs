    ...
    public class PGNLoader
    {
		...
		static Move MoveFromAlgebraic (Board board, string algebraicMove) {
			...
			foreach (Move moveToTest in allMoves) {
				...
				int movePieceType = Piece.GetPieceType(board.squares[moveFromIndex]);
				...
				else if (BoardRepresentation.FileNames.Contains (algebraicMove[0].ToString ())) {
					if (movePieceType != Piece.Pawn) {
						continue;
					}
					if (BoardRepresentation.FileNames.IndexOf (algebraicMove[0]) == fromCoord.fileIndex) { // correct starting file
